  ...
  return (
    <React.Fragment key={instructor.fullName}>
      ...
      <Modal
        ...
        size="4xl"
        classNames={{ body: "flex xl:flex-row !w-full !h-fit",footer: "p-0 ",header: "px-0 "}}
      >
        <ModalContent>

          <ModalBody>
          <ScrollShadow className="flex flex-col sm:flex-row w-full h-fit " hideScrollBar>
            <div className="flex  w-full sm:max-w-md lg:max-w-xl ">
              <Image
                src={instructor.portfolioImages[0].image}
                alt={instructor.fullName}
                radius="none"
            ...
            <div className="flex flex-col w-full p-0 md:px-4">
              <ModalHeader>
                <h4 className={`pb-3 !font-bold ${title({ size: "xl" })}`}>
                  {instructor.fullName}
                </h4>
              </ModalHeader>
              <p
              ...
              <p
                className={`before:text-default-500 before:font-normal before:text-sm  before:content-['Gender__:__'] font-semibold ${paragraph(
                  { size: "md" }
                )}`}
              >
                {instructor.gender}
              ...
              <div className="flex flex-col border border-default-200 rounded-md relative my-4 p-2">
                <p
                  className={`absolute left-2 -top-3.5 bg-default-50 rounded-full font-semibold  ${paragraph(
                    { size: "sm" }
                  )}`}
                >
                  Specializes
             ...
             {instructor.experience && instructor.experience.length > 0 ? (
              <div className="flex flex-col border border-default-200 rounded-md relative my-4 p-2">
                ...
                { instructor.experience.map((experience) => (
                      ...
                      >
                        <p
                          className={`before:text-default-500 before:font-normal before:text-sm  before:content-['Company__:__'] font-semibold ${paragraph(
                            { size: "sm" }
                          )}`}
                        >
                          {experience.company}
                        ...
                        <p
                          className={`before:text-default-500 before:text-sm  before:content-['Position__:__']  ${paragraph(
                            { size: "sm" }
                          )}`}
                        >
                          {experience.position}
                  ...
                  <div className="flex flex-col border border-default-200 rounded-md relative my-4 p-2 empty:hidden">
                    ...
                    {instructor.education.map((education) => (
                      <div className="flex flex-col w-full px-2 py-2" key={education.school}>
                        <p
                          className={`before:text-default-500 before:text-sm  before:content-['School__:__']  ${paragraph(
                            { size: "sm" }
                          )}`}
                        >
                          {education.school}
                        ...
                        <p
                          className={`before:text-default-500 before:text-sm  before:content-['Degree__:__']  ${paragraph(
                            { size: "sm" }
                          )}`}
                        >
                          {education.degree}
